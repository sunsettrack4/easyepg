				...
				if( $cidEXTnew->{$cid} eq $selected_channel ) {
					if( $setup_cid eq $enabled ) {
						if( defined $rytec->{$cidEXTnew->{$cid}} ) {
							print "<programme start=\"$start +0000\" stop=\"$end +0000\" channel=\"" . $rytec->{$cidEXTnew->{$cid}} . "\">\n";
						} else {
					...
					if( $cidEXTold->{$cid} eq $selected_channel ) {
						if( $setup_cid eq $enabled ) {
							if( defined $rytec->{$cidEXTold->{$cid}} ) {
								print "<programme start=\"$start +0000\" stop=\"$end +0000\" channel=\"" . $rytec->{$cidEXTold->{$cid}} . "\">\n";
							} else {
								print "<programme start=\"$start +0000\" stop=\"$end +0000\" channel=\"" . $cidEXTold->{$cid} . "\">\n";
