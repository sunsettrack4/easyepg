...
my @newchannels_name2logo = @{ $newdata->{'channellist'} };
...
foreach my $newchannels ( @newchannels_name2logo ) {
	...
	if( @logo ) {
		...
		if( defined $image_location ) {
			my $logo_def = $newchannels->{'pictures'}[$image_location]{'href'};
			print $logo_def . "\",\n";
		} else {
			print "\",\n";
		}
...
my @oldchannels_name2logo = @{ $olddata->{'channellist'} };
...
foreach my $oldchannels ( @oldchannels_name2logo ) {
	...
	if( @logo ) {
		...
		if( defined $image_location ) {
			my $logo_def = $oldchannels->{'pictures'}[$image_location]{'href'};
			print $logo_def . "\",\n";
		} else {
			print "\",\n";
		}
