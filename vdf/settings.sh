...
do
	# V1000 MENU OVERLAY
	echo 'dialog --backtitle "[V1000] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS" --title "SETTINGS" --menu "Please select the option you want to change:" 14 60 10 \' > /tmp/menu 
...
#	fi
	...
	# V1700 RUN XML SCRIPT
	echo '	7 "RUN XML SCRIPT" \' >> /tmp/menu
	
	# V1900 DELETE INSTANCE
	...
	then
		# V1100 MENU OVERLAY
		echo 'dialog --backtitle "[V1100] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CHANNEL LIST" --title "CHANNELS" --checklist "Please choose the channels you want to grab:" 15 50 10 \' > /tmp/chmenu
		
		printf "\rFetching channel list... "
		
		if ! curl --write-out %{http_code} --silent --output /dev/null https://tv-manager.vodafone.de/tv-manager/backend/auth-service/proxy/epg-data-service/epg/tv/channels | grep -q "200"
		then
		...
		then
			...
			then
				...
				dialog --backtitle "[V1110] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CHANNEL LIST" --title "INFO" --msgbox "New channel list added!\nPlease run the grabber to add the channels to the setup modules!" 7 50
				echo "H" > /tmp/value
			else
				dialog --backtitle "[V1120] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CHANNEL LIST" --title "INFO" --msgbox "Channel list creation aborted!\nPlease note that at least 1 channel must be included in channel list!" 7 50
				echo "M" > /tmp/value
				exit 1
			fi
		else
			...
			then
				sed 's/" "/","/g;s/\\\[[0-9][^]]*\] //g;s/\\(/(/g;s/\\)/)/g;s/.*/{"channels":[&]}/g;s/\\\&/\&amp;/g;s/\\//g' /tmp/chconf > channels.json
				dialog --backtitle "[V1130] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CHANNEL LIST" --title "INFO" --msgbox "New channel list saved!\nPlease run the grabber to add new channels to the setup modules!" 7 50
				cp /tmp/chlist chlist_old
				echo "H" > /tmp/value
			else
				dialog --backtitle "[V1140] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CHANNEL LIST" --title "INFO" --msgbox "Channel list creation aborted!\nPlease note that at least 1 channel must be included in channel list!" 7 50
				echo "H" > /tmp/value
			fi
		fi
	...
	then
		...
		do
			# V1200 MENU OVERLAY
			dialog --backtitle "[S1200] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > TIME PERIOD" --title "EPG GRABBER" --inputbox "Please enter the number of days you want to retrieve the EPG information. (0=disable | 1-14=enable)" 10 46 2>/tmp/value
							
			sed -i 's/.*/epg&-/g' /tmp/value
			
			# V1210 INPUT: DISABLED
			if grep -q "epg0-" /tmp/value
			then
				sed -i '/day=/d' /tmp/settings_new
				echo "day=0" >> /tmp/settings_new
				dialog --backtitle "[S1210] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > TIME PERIOD" --title "INFO" --msgbox "EPG grabber disabled!" 5 26 
				echo "H" > /tmp/value
			...
			then
				...
				dialog --backtitle "[S1220] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > TIME PERIOD" --title "INFO" --msgbox "EPG grabber is enabled for 1 day!" 5 42
				echo "H" > /tmp/value
				
			# V1230 INPUT: 2-9 DAYS
			elif grep -q "epg[2-9]-" /tmp/value
			then
				sed -i 's/epg//g;s/-//g' /tmp/value
				sed -i '/day=/d' /tmp/settings_new
				echo "day=$(</tmp/value)" >> /tmp/settings_new
				dialog --backtitle "[S1230] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > TIME PERIOD" --title "INFO" --msgbox "EPG grabber is enabled for $(</tmp/value) days!" 5 42
				echo "H" > /tmp/value
			
			# V1240 INPUT: 10-14 DAYS
			elif grep -q "epg1[0-4]-" /tmp/value
			then
				sed -i 's/epg//g;s/-//g' /tmp/value
				sed -i '/day=/d' /tmp/settings_new
				echo "day=$(</tmp/value)" >> /tmp/settings_new
				dialog --backtitle "[S1240] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > TIME PERIOD" --title "INFO" --msgbox "EPG grabber is enabled for $(</tmp/value) days!" 5 42
				echo "H" > /tmp/value
			
			# V1250 WRONG INPUT
			elif [ -s /tmp/value ]
			then
				dialog --backtitle "[S1250] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > TIME PERIOD" --title "ERROR" --msgbox "Wrong input detected!" 5 30 
				echo "X" > /tmp/value
	...
	then
		# V1300 MENU OVERLAY
		dialog --backtitle "[V1300] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CONVERT CHANNEL IDs" --title "CHANNEL IDs" --yesno "Do you want to use the Rytec ID format?\n\nRytec ID example: ChannelNameHD.de\nUsual ID example: Channel Name HD" 8 55
		...
		if [ $response = 1 ]
		then
			dialog --backtitle "[V1310] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CONVERT CHANNEL IDs" --title "INFO" --msgbox "Rytec Channel IDs disabled!" 5 32
		...
		then
			dialog --backtitle "[V1320] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CONVERT CHANNEL IDs" --title "INFO" --msgbox "Rytec Channel IDs enabled!" 5 30
			sed -i '/cid=/d' /tmp/settings_new
		...
		then
			dialog --backtitle "[V13X0] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CONVERT CHANNEL IDs" --title "INFO" --msgbox "No changes applied!" 5 30
			echo "H" > /tmp/value
		fi
	...
	then
		# V1400 MENU OVERLAY
		dialog --backtitle "[V1400] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CONVERT CATEGORIES" --title "CATEGORIES" --yesno "Do you want to use the EIT format for tvHeadend?" 5 55
		...
		if [ $response = 1 ]
		then
			dialog --backtitle "[V1410] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CONVERT CATEGORIES" --title "INFO" --msgbox "EIT categories disabled!" 5 32
		...
		then
			dialog --backtitle "[V1420] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CONVERT CATEGORIES" --title "INFO" --msgbox "EIT categories enabled!" 5 30
			sed -i '/genre=/d' /tmp/settings_new
		...
		then
			dialog --backtitle "[V14X0] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > CONVERT CATEGORIES" --title "INFO" --msgbox "No changes applied!" 5 30
			echo "H" > /tmp/value
		fi
	...
	then
		# V1500 MENU OVERLAY
		dialog --backtitle "[V1500] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > MULTIPLE CATEGORIES" --title "MULTIPLE CATEGORIES" --yesno "Do you want to use multiple categories for tvHeadend?" 5 60
		...
		if [ $response = 1 ]
		then
			dialog --backtitle "[V1510] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > MULTIPLE CATEGORIES" --title "INFO" --msgbox "Multiple categories disabled!" 5 35
		...
		then
			dialog --backtitle "[V1520] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > MULTIPLE CATEGORIES" --title "INFO" --msgbox "Multiple categories enabled!" 5 35
			sed -i '/category=/d' /tmp/settings_new
		...
		then
			dialog --backtitle "[V15X0] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > MULTIPLE CATEGORIES" --title "INFO" --msgbox "No changes applied!" 5 30
			echo "H" > /tmp/value
		fi
	...
	then
		# V1600 MENU OVERLAY
		dialog --backtitle "[V1600] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > EPISODE FORMAT" --title "EPISODE" --menu "Please select the format you want to use.\n\nonscreen: move the episode data into the broadcast description\nxmltv_ns: episode data to be parsed by tvHeadend" 14 60 10 \
		1	"ONSCREEN" \
		2	"XMLTV_NS" \
		2>/tmp/value
		...
		if grep -q "1" /tmp/value
		then
			dialog --backtitle "[V1610] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > EPISODE FORMAT" --title "INFO" --msgbox "Episode format 'onscreen' enabled!" 5 40
		...
		then
			dialog --backtitle "[V1620] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > EPISODE FORMAT" --title "INFO" --msgbox "Episode format 'xmltv_ns' enabled!" 5 40
			sed -i '/episode=/d' /tmp/settings_new
	...
	then
		# V1900 MENU OVERLAY
		dialog --backtitle "[V1900] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > DELETE INSTANCE" --title "WARNING" --yesno "Do you want to delete this service?" 5 50
		...
		if [ $response = 1 ]
		then
			dialog --backtitle "[V1910] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > DELETE INSTANCE" --title "INFO" --msgbox "Service not deleted!" 5 32
			echo "H" > /tmp/value
						
		# V1920 YES
		elif [ $response = 0 ] 
		then
			dialog --backtitle "[V1920] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > DELETE INSTANCE" --title "INFO" --msgbox "Service deleted!" 5 30
			rm channels.json
			echo "M" > /tmp/value
		...
		then
			dialog --backtitle "[V19X0] EASYEPG SIMPLE XMLTV GRABBER > VODAFONE SETTINGS > DELETE INSTANCE" --title "INFO" --msgbox "Service not deleted!" 5 30
			echo "H" > /tmp/value
		fi
